package advent_of_code.years.year2022.day05;
...
public class Day05 extends RootDay {
    public Day05(Year year, int day) {
        super(year, day, "QMBMJDFTD", "NBTVTJNFJ");
    ...
    private static void moveFromInput(ArrayList<ArrayList<String>> stacks, String input, boolean moveStack) {
        for (String line : input.split("\n")) {
            if (line.contains("move")) {
                int nr = Integer.parseInt(line.replaceAll("move ", "").replaceAll("from .*", "").trim());
                int fromIndex = Integer.parseInt(line.replaceAll(".* from", "").replaceAll("to .*", "").trim()) -1;
                int toIndex = Integer.parseInt(line.replaceAll(".* to ", "").trim()) - 1;
